...
// handleDownloadOsqueryInstallScript serves the osquery agent installation script
func (r *Router) handleDownloadOsqueryInstallScript(w http.ResponseWriter, req *http.Request) {
	...
	if err != nil {
		...
		content, err = os.ReadFile(scriptPath)
		if err != nil {
			log.Error().Err(err).Str("path", scriptPath).Msg("Failed to read osquery agent installer script")
			http.Error(w, "Failed to read installer script", http.StatusInternalServerError)
			return
		}
	}

	http.ServeContent(w, req, "install-osquery-agent.sh", time.Now(), bytes.NewReader(content))
}

// handleDownloadOsqueryAgent serves the osquery agent binary
func (r *Router) handleDownloadOsqueryAgent(w http.ResponseWriter, req *http.Request) {
	...
	// Build search paths for osquery agent binary
	searchPaths := make([]string, 0, 12)
	strictMode := platformParam != "" && archParam != ""

	if strictMode {
		searchPaths = append(searchPaths,
			filepath.Join(pulseBinDir(), fmt.Sprintf("pulse-osquery-agent-%s-%s", platformParam, archParam)),
			filepath.Join("/opt/pulse", fmt.Sprintf("pulse-osquery-agent-%s-%s", platformParam, archParam)),
			filepath.Join("/app", fmt.Sprintf("pulse-osquery-agent-%s-%s", platformParam, archParam)),
		)
	}

	if platformParam != "" && !strictMode {
		searchPaths = append(searchPaths,
			filepath.Join(pulseBinDir(), "pulse-osquery-agent-"+platformParam),
			filepath.Join("/opt/pulse", "pulse-osquery-agent-"+platformParam),
			filepath.Join("/app", "pulse-osquery-agent-"+platformParam),
		)
	}

	if !strictMode && platformParam == "" {
		searchPaths = append(searchPaths,
			filepath.Join(pulseBinDir(), "pulse-osquery-agent"),
			"/opt/pulse/pulse-osquery-agent",
			filepath.Join("/app", "pulse-osquery-agent"),
		)
	}
	...
	http.Error(w, fmt.Sprintf("Osquery agent binary not found for %s/%s", platformParam, archParam), http.StatusNotFound)
}

// handleDownloadOsqueryAgentChecksum serves the osquery agent binary checksum
